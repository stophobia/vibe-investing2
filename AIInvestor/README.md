...
- **응답 속도**: Korean retail 가중 평균 ~1.7초. Tier 1 (commentary cache hit) ~1초 / Tier 2 (snapshot hit) ~3.5초 / cold ~5초. 자세한 분해는 [postmortem.md](postmortem.md) 참조.
...
함께 보는 자료:
- **[postmortem.md — 응답 지연 진단·수정 + 사전 캐싱 도입 기록 (1+2부)](postmortem.md)** — 배포 11회 (성공 7 / 실패 4) + App Insights timeline 분해 + 핵심 병목(`async with` per-request, DeepSeek body 스트리밍) 식별 + 사전 캐싱 (Tier 1 commentary / Tier 2 snapshot) 도입으로 가중 평균 5초 → 1.7초. Azure Free Trial Front Door 제약, `aiohttp` transitive 의존 등 함정 체크리스트 포함.
...
├── function_app.py               Azure Functions 진입점 (webhook + daily_report + 2개 prewarm timer + keepalive)
├── infra/main.bicep              Azure 인프라 — Storage + KV + Functions Flex + App Insights + (선택) Front Door
