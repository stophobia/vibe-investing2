...
### Phase 6 — 챗봇 분석
- [ ] LLM 기반 챗봇 인터페이스 (자연어 질의)
- [ ] "A기업과 B인물의 관계 분석해줘" → 연관도 + 이벤트 타임라인 응답
- [ ] 특정 기업·인물·법인·조합의 과거 코스닥 상장사 이벤트 연관도 분석
- [ ] 뉴스 크로스레퍼런스 + 공시 패턴 기반 리스크 평가
- [ ] "현재 공시 패턴의 위험도를 평가해줘" → 신호 점수 + 근거 요약
- [ ] 스트리밍 응답 (DeepSeek/Claude)
