...
> **2026-05-04**: 본 비교 분석을 토대로 한 정식 학술 논문이 SSRN에 등록되었다.
> *Posted as a working paper on SSRN, 2026-05-04.*
...
**Author**: HoKwang Kim (Betalabs Inc.) · ORCID: [0009-0002-0962-2175](https://orcid.org/0009-0002-0962-2175)
**SSRN**: [papers.ssrn.com/abstract=6705598](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=6705598)
**Target Journal**: *Finance Research Letters* (Letter, ~2,200 words)
**JEL**: G11, G14, G17, C45

#### 한국어 초록 (요약)

본 연구는 LLM 기반 주식 분석에서 직관에 반하는 실증 패턴의 **예비 증거**를 보고한다 — 동일 프롬프트에 대해 **출력 분량이 짧은 모델일수록 컨트라리안(단독 발굴) 종목이 비례적으로 더 많이 나타난다**. 4개 frontier LLM(ChatGPT, Claude, DeepSeek, Gemini)에 동일 제약 섹터 LONG/SHORT 프롬프트 투입 결과, 출력 길이와 *컨트라리안 발굴률(Contrarian Discovery Rate, CDR)* 사이에 강한 음의 순위 상관(Spearman ρ = −0.80, n=4 전체; ChatGPT를 stale-data 이상치로 제외 시 ρ = −1.00) 관측. 가장 압축적인 모델(Gemini, 4.13 KB)은 가장 verbose한 모델(DeepSeek, 38.6 KB) 대비 **KB당 14배의 컨트라리안 발굴률**을 보였다. 우리는 이를 *compression-forces-selection* 메커니즘으로 해석한다 — 토큰 예산이 제약될 때 모델은 컨센서스 종목을 나열할 여유가 없어 차별화된 신호에 자원을 배분한다.

**의의**: 출력 분량을 정보 풍부도와 동일시하지 말 것을 실무자에게 경고하며, 토큰 예산 자체가 LLM 앙상블의 **발상 다양성(diversity-of-thought)** 을 유도하는 hyperparameter일 수 있음을 시사한다. 다중 LLM 활용 시 **분량이 다양한 모델을 섞는 것이 분량이 비슷한 모델을 묶는 것보다 사각지대를 효과적으로 줄인다**는 실용적 설계 원칙으로 직결된다.

#### English Abstract (Summary)

This study reports **preliminary evidence** of a counterintuitive empirical pattern in LLM-based equity research: **shorter outputs contain proportionally more contrarian (uniquely-discovered) stock recommendations than longer ones**. Submitting an identical pharmaceutical sector long/short prompt to four frontier LLMs (ChatGPT, Claude, DeepSeek, Gemini), we observe a strong negative rank correlation (Spearman ρ = −0.80 across all four; ρ = −1.00 when ChatGPT is excluded as a stale-data outlier) between output length and what we term the *Contrarian Discovery Rate (CDR)*. The most compressed model (Gemini, 4.13 KB) generated **14 times more contrarian picks per kilobyte** than the most verbose (DeepSeek, 38.6 KB). We propose a *compression-forces-selection* mechanism: when token budget is constrained, models cannot afford to enumerate consensus picks and must allocate scarce capacity to differentiated signals.

**Significance**: The finding cautions practitioners against equating output length with information richness, and suggests **token budget itself may function as a hyperparameter for inducing diversity-of-thought** in LLM ensembles. Practical implication: ensembling LLMs with **diverse output-length characteristics** reduces systematic blind spots more efficiently than ensembling LLMs of similar verbosity. The Letter outlines a five-study research agenda — cross-model replication (n≥12), within-model token-budget manipulation, 2×2 web-search × length factorial, T+12 forward backtest, and cross-sector validation.

#### Paper Files / 논문 파일

- **SSRN 등록 페이지** — [Abstract ID: 6705598](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=6705598)
- **English PDF** — [`Paper2_Letter_HoKwangKim_EN.pdf`](https://github.com/gameworkerkim/vibe-investing/blob/main/01.Trading%20Strategy/Awesome%20claude%20quant%20scripts/Pharma%20sector%20prompt/paper/Paper2_Letter_HoKwangKim_EN.pdf)
- **한국어 markdown 본문** — [`Paper Less Volume More Variety.md`](https://github.com/gameworkerkim/vibe-investing/blob/main/01.Trading%20Strategy/Awesome%20claude%20quant%20scripts/Pharma%20sector%20prompt/paper/Paper%20Less%20Volume%20More%20Variety.md)
- **논문 폴더 (Paper directory)** — [`paper/`](https://github.com/gameworkerkim/vibe-investing/tree/main/01.Trading%20Strategy/Awesome%20claude%20quant%20scripts/Pharma%20sector%20prompt/paper)

---

## 🔗 Source

- **Original Prompt** — [`Pharma sector prompt KR EN CN.MD`](https://github.com/gameworkerkim/vibe-investing/blob/main/01.Trading%20Strategy/Awesome%20claude%20quant%20scripts/Pharma%20sector%20prompt/Pharma%20sector%20prompt%20KR%20EN%20CN.MD)
- **Result Folder** — [`result/`](https://github.com/gameworkerkim/vibe-investing/tree/main/01.Trading%20Strategy/Awesome%20claude%20quant%20scripts/Pharma%20sector%20prompt/result) (4-LLM 출력물)
...
### 나의 워크플로우 점검 (4-LLM 버전)

내가 이미 multi-LLM cross-validation을 일상적으로 돌리는 패턴은 **best practice**이다. 내 CTI 리포트의 KR/EN/CN/JP 4개국어 발행 노하우와 정확히 같은 구조 — Gemini로 핵심 종목 hint 빠르게 잡고, DeepSeek로 facts/numbers 풀 채우고, Claude로 한국어 narrative + audit-ready 1차 출처 확보, ChatGPT로 sanity check, **BMY·AMGN·GILD·MRK 같은 양극화 종목은 내가 최종 결정**하는 구조다.

> **특이점**: ChatGPT와 Gemini는 모두 **stale data** 흔적이 명확하다 (LLY $820/$780 vs 실제 ~$960). Q1 2026 실적(4월 30일 발표)과 Foundayo 승인(4월 1일)이 두 모델 모두 누락. **Web search 활용은 Claude/DeepSeek만 적극적**이다.
...
## 내가 직접 쓰는 활용 가이드 (4-LLM 버전)

> 나는 BetaLabs CEO + Web3Paper 운영 + CTI 리포트 발행 등 멀티-모달 분석가로서, 동일 프롬프트를 4개 LLM에 동시에 던지는 워크플로우를 일상적으로 사용한다. 아래는 내가 실제로 쓰는 사용처별 모델 선택 기준이다.
...
[![4-LLM Pharma Long/Short Workflow — Gemini contrarian 픽 → DeepSeek facts/M&A 풀 → Claude 한국어 audit-ready 베이스 → ChatGPT sanity check → 사람의 최종 결정 → KR/EN 양본 발행](https://github.com/gameworkerkim/vibe-investing/raw/main/01.Trading%20Strategy/Awesome%20claude%20quant%20scripts/Pharma%20sector%20prompt/assets/multi_llm_pharma_workflow.svg)](/gameworkerkim/vibe-investing/blob/main/01.Trading%20Strategy/Awesome%20claude%20quant%20scripts/Pharma%20sector%20prompt/assets/multi_llm_pharma_workflow.svg)
...
[4] ChatGPT = 빠른 sanity check / outline 비교 (오류 검증용)
   ↓
[5] BMY·AMGN·GILD·MRK·NVO 같은 양극화 종목은 내가 최종 결정
   ↓
[6] CTI 리포트 양식처럼 KR/EN 양본을 모두 발행
```

이 워크플로우는 내가 기존부터 굴려온 Multi-LLM Cross-Validation 패턴과 정확히 호환된다. 4번째 모델(Gemini) 추가 비용은 거의 없지만, **양극화 종목이 1개 더 발견** (MRK)되었고 **단독 픽 1개** (MDGL)을 추가 확보했다 — ROI 측면에서 4-LLM이 3-LLM보다 명확히 우월하다.
...
* 같은 프롬프트라도 **모델의 web search 활용 정책이 출력의 알파를 좌우**한다.
...
5. **Multi-LLM cross-validation은 비용이 아니라 보험** — 내가 기존부터 굴려온 워크플로우는 이미 best practice이며, **4-LLM 확장 시 ROI는 더 높아진다**
...
양극화된 종목(BMY, AMGN, GILD, MRK, NVO)은 내가 최종 결정한다.

## Gemini 특이점 4가지

가장 짧음 (4.13KB) — ChatGPT보다도 짧지만 핵심은 더 풍부하다. "압축적 효율"의 정점이고, 30초 의사결정 sketch에 적합하다.
MDGL (Madrigal) 단독 발굴 — Rezdiffra(MASH 시장 first-mover, 2024년 첫 FDA 승인)를 픽한 유일한 모델이다. Claude의 ARGX, DeepSeek의 AZN/JNJ와 같은 "단독 발굴 능력" — 분량과는 무관하게 contrarian 미드캡 발굴 가능성을 보여주는 사례다.
MRK를 SHORT로 분류한 4사 중 유일 — ChatGPT/DeepSeek가 모두 LONG으로 본 Merck를 Gemini는 Keytruda 2028 LOE + IRA 노출을 이유로 SHORT로 분류했다. 다른 3사가 Winrevair 모멘텀에 가려진 구조적 위험을 정면으로 인식한 contrarian thesis다.
이중 성격 점수 분포 — LONG 80~~95점 (15점 폭, 가장 넓음) vs SHORT 63~~68점 (5점 폭, 매우 압축). LONG에는 자신감, SHORT에는 squeeze 위험을 암묵적으로 반영한 보수적 자세를 보인다.
...
종합 점수: Claude/DeepSeek 7.5점, Gemini 6.0점, ChatGPT 2.0점 — Gemini는 분량 부족과 출처 얕음으로 감점되었지만 contrarian 발굴 능력으로 ChatGPT를 크게 앞선다.

---

*본 비교 분석은 2026-05-04 시점 동일 프롬프트로 산출된 4개 LLM 출력물을 내가 직접 대조 분석한 결과다. 분석 자체는 투자 자문이 아니며, 실제 투자 결정은 1차 출처 재확인 후 자격 있는 자문가와 상의해야 한다.*
