...
**작성일**: 2026-06-06 · **버전**: v1.1 · **라이선스**: MIT

> **v1.1 변경점** (피드백 반영): ① **금리 스트레스 축** + 하락유형 라벨(침체형/금리형/밸류형) 추가 — "침체 Composite는 낮은데 왜 떨어지나(금리 쇼크)"를 구분하고 ARDS Tier2 TLT/IEF 헤지 작동 여부를 조건부로 안내. ② **히스테리시스**(진입/이탈 밴드 + N일 확인)로 레짐 핑퐁/휩쏘 방지. ③ 백테스트에 **신뢰도 캘리브레이션(Brier)** 추가. 자세한 검증 결과는 §6.5.
...
### 4.1 금리 스트레스 축 + 하락유형 라벨 (v1.1)

2축(거시 침체 × 가격 스트레스)만으로는 *"침체 Composite는 낮은데 왜 떨어지나"* 를 설명 못 합니다 — **금리 쇼크**가 그 사각지대입니다. 그래서 **Rate Stress 서브컴포지트**를 더해 하락의 *원인*을 라벨링합니다.

**Rate Stress (0~100)** = `0.35·R1 + 0.25·R2 + 0.20·R3 + 0.20·R4`
- R1 10Y 금리 20일 변화(bp) · R2 2Y(또는 5Y) 변화 · R3 5Y 기대인플레(브레이크이븐) 변화 · R4 MOVE(또는 10Y 실현변동성)

**하락유형 라벨** (가격 스트레스가 유의미할 때만):

| 라벨 | 조건 | TLT/IEF 처방 |
|---|---|---|
| **침체형** `RECESSION_DRIVEN` | 거시 Composite ≥ 55 | TLT/IEF 듀레이션 = **헤지** (금리 인하 수혜). ARDS Tier2 장기국채 활성화 OK |
| **금리형** `RATE_DRIVEN` | Rate Stress ≥ 55, 거시 < 55 | ⚠️ TLT/IEF = **추가 익스포저** (주식과 동반 하락). 장기국채 축소 → 단기채(BIL/SHV)·금(GLD) 대체 |
| **밸류에이션형** `VALUATION_DRIVEN` | 둘 다 미만 | 멀티플 압축/크라우딩 청산. TLT 헤지 제한적, 현금·인버스(SH/PSQ)가 적합 |

> 처방이 정반대인 이유 = ARDS 원본의 *"disinflation recession vs sticky-inflation recession"* 논쟁(README §5.4)과 동일선상. 백테스트에서 **금리형 하락의 향후 60일 수익률은 -7.3%** 로, 침체 축이 놓치는 진짜 위험 구간을 라벨이 분리해 냅니다(§6.5).

### 4.2 히스테리시스 (휩쏘 방지, v1.1)

경계에서 RSI가 흔들리면 레짐이 핑퐁됩니다. 두 가지로 막습니다:
- **진입/이탈 밴드 분리**: 예) 과매도는 RSI 30 *진입* / 38 *이탈* — 30~38 사이 핑퐁 제거. 드로다운·거시 침체에도 동일.
- **N일 확인**: 새 raw 레짐이 **2거래일 연속** 유지돼야 공식(committed) 레짐 전환. 그 전까지는 `전환대기` 로 표시되고 신뢰도를 하향.

백테스트 결과 레짐 전환 횟수가 **249 → 88회(-65%)** 로 감소했습니다(§6.5).
...
### (a) 상태별 향후 수익률 — 가설: 조정/과매도는 +, 하락/침체는 − (히스테리시스 적용)

| 상태 | 비중(시간) | 향후 20일 | 향후 60일 | 60일 승률 | 판정 |
|---|---|---|---|---|---|
| 정상 상승 | 65.4% | +1.06% | +2.96% | 70.8% | ✅ |
| **조정** | 13.0% | +2.48% | **+5.32%** | 74.9% | ✅ 저점매수 유효 |
| **단기 과매도** | 1.1% | +0.67% | **+6.45%** | 87.9% | ✅ 반등 유효 |
| **하락 / 분배** | 5.0% | +0.12% | **−1.52%** | 49.0% | ✅ 경계 정당 |
| 침체 리밸런싱 | 15.5% | +3.01% | **+10.92%** | 85.8% | ⚠️ **역신호** |

> **핵심 발견**: 조정·과매도·하락 분류는 가설대로 작동(조정/과매도 60일 +5~6%대, 하락 구간 마이너스). 그러나 **`침체 리밸런싱` 은 향후 수익률이 가장 높습니다(+11%)** — *시장 프록시만으로 만든 거시 축이 2022–24 곡선 역전 내내 침체를 과다 발화(BEAR_TRAP)* 하기 때문. **실시간판이 FRED Sahm Rule·LEI 실데이터를 쓰는 이유**입니다. → 이 한계를 **(d) 금리형 라벨이 직접 보완**합니다.
...
| **ARDS-X 스위치** | +200.0% | 10.0% | 15.5% | 0.69 | **−30.5%** | 79% |

> 2014–2026 강세장에서 방어 오버레이는 수익을 깎는 대신 MDD/변동성을 낮춥니다("강세장에서 방어는 세금"). ARDS-X 는 단독 알파가 아니라 **레짐 스위치**이므로 이는 "방어 비용"의 참고치입니다.
...
> 진입/이탈 밴드 + 2일 확인만으로 불필요한 전환의 3분의 2가 사라집니다 — 경계 핑퐁 휩쏘 직접 제거.

### (d) 하락유형 라벨 — 금리형이 진짜 위험을 분리해 낸다 ★

| 하락유형 | 일수 | 향후 20일 | 향후 60일 |
|---|---|---|---|
| 침체형 `RECESSION_DRIVEN` | 301 | +3.70% | +11.46% (프록시 BEAR_TRAP) |
| **금리형** `RATE_DRIVEN` | 81 | **−1.84%** | **−7.27%** |
| 밸류에이션형 `VALUATION_DRIVEN` | 312 | +1.98% | +5.11% |

> **이번 추가의 핵심 성과**: 침체 축이 과다발화로 놓치는 진짜 위험 구간을, **금리형 라벨이 향후 60일 −7.3% 로 정확히 분리**해 냅니다. 즉 "Composite는 낮은데 떨어지는" 하락의 가장 위험한 부분집합을 새 축이 식별합니다. (이 구간에서 TLT는 헤지가 아니라 추가 손실원 → Tier2 조건부 게이팅의 근거.)

### (e) 신뢰도 캘리브레이션 (Brier = 0.281)

"신뢰도 X%가 실제로 X% 적중하는가"를 검증. 정답 = risk-on이면 향후20일 +, 방어면 −.
...
| 60–70% | 538 | 62.7% | **59.9%** ✅ |
| 70–80% | 237 | 77.6% | 68.8% |
| 80–95% | 1804 | 86.9% | **61.0%** ⚠️ 과신 |

> **정직한 결과**: 60–70% 구간은 잘 보정돼 있으나, **상단(80%+)은 과신**(86.9% 예측 vs 61.0% 실제)입니다. 즉 현재 신뢰도 점수는 고확신 구간에서 과대평가되며, 이는 *향후 보정(recalibration) 과제*입니다 — 본 자체가 SSRN 워킹페이퍼급 연구 트랙(2024-08 엔캐리·2025-04 관세 쇼크가 정성 사례).

실행: `python backtest.py` → `backtest_state_stats.csv` · `backtest_equity.csv` · `backtest_calibration.csv` · `backtest_decline_types.csv` 생성.
...
│   └── ARDS-X_cn.MD         ← 中文
├── quant/
│   ├── config.py            ← 유니버스·가중치·임계값·금리·히스테리시스
│   ├── datafeed.py          ← yfinance + FRED 무료 CSV (캐시·폴백)
│   ├── macro.py             ← 5-Factor Recession Composite (실데이터)
│   ├── rates.py             ← Rate Stress 서브컴포지트 + 하락유형 라벨 (v1.1)
│   ├── technical.py         ← 드로다운·추세·과매도·폭 지표
│   ├── classifier.py        ← 레짐 분류 엔진 + 히스테리시스 (핵심)
│   ├── run.py               ← 파이프라인 → latest.json
│   ├── backtest.py          ← 과거 검증 (히스테리시스·하락유형·캘리브레이션)
│   ├── backtest_state_stats.csv   ← 상태별 향후 수익률
│   ├── backtest_decline_types.csv ← 하락유형별 향후 수익률 (v1.1)
│   ├── backtest_calibration.csv   ← 신뢰도 캘리브레이션 (v1.1)
